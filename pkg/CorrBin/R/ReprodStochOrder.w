...
We will be using object of \texttt{CBData} class, so we load the file that defines
it.
@o Reprod.R
@{
  source("CBData.R")
...
treatment group. In order to use the C code, it has to be first compiled into a
DLL by calling \texttt{Rcmd SHLIB ReprodCalcsFinal.c} from console. Then we load it
...
@o Reprod.R
@{
  dyn.load("ReprodCalcsFinal.dll")
@}


@o Reprod.R
@{
reprod.est <- function(cbdata){
  #by trt
...
The \texttt{reprod.test.chisq} function implements Pang and Kuk's version of
...
@O Reprod.R
@{
#expected.n.unobserved <- function(cbdata){
#    #vector of expected number of clusters 
#    #with t responses (observed or unobserved)
#    #out of max.size
#    
#    #treats data as one group (should be applied by treatment levels)
#   cbdata <- subset(cbdata, Freq>0)
#   max.size <- max(cbdata$ClusterSize)    
#   est <- .Call("ReprodEstimates", as.integer(cbdata$ClusterSize), as.integer(cbdata$NResp), 
#	              as.integer(cbdata$Freq))
#   est.m <- est[,max.size]
#   comb.mat.list <- lapply(1:max.size, function(rr){
#       outer(0:max.size, 0:max.size, function(tt,ss){
#			     choose(tt,ss)*choose(max.size-tt, rr-ss)})})
#   p.rst <- lapply(comb.mat.list, function(x){
#	      y <- crossprod(diag(est.m), x); y %*% diag(1/colSums(y))})
#   match.p.rst <- function(data.row,tt){
#	      p.rst[[data.row[["ClusterSize"]]]][tt+1,data.row[["NResp"]]+1]
#				}
#   n.t <- sapply(0:max.size, function(tt){
#	      sum(cbdata$Freq*apply(data.matrix(cbdata),1,match.p.rst,tt=tt))})
#   n.t}
#   
   
reprod.test.chisq <- function(cbdata){
  cbdata <- subset(cbdata, Freq>0)
  ...
  get.T <- function(x){
      ...
      #tt <- 0:max.size
      #E.Y1Y2 <- sum( tt*(tt-1)*n.t/(max.size*(max.size-1)*K))
      #rho.hat <- (E.Y1Y2 - p.hat^2)/(p.hat*(1-p.hat))
...
mix.reprod.mle <- function(cbdata, turn=1, control=mixControl()){ 
  attach(control)
...
the control argument to the \texttt{mix.reprod.mle} function.
...
The \texttt{null.LRT} computes the likelihood-ratio test statistic.
@O Reprod.R
@{
null.LRT <- function(cbdata, control=mixControl()){
  ...
  res <- mix.reprod.mle(cbdata, control=control)
  ll1 <- res$loglik
  lrt <- 2*(ll1 - ll0)
  attr(lrt, "ll0") <- ll0
  attr(lrt, "ll1") <- ll1
...
To get the p-value for the LRT, we use a permutation testing approach. The \texttt{perm.LRT} function
...
\subsection{Testing for hormesis (umbrella ordering)}

Stochastic ordering is a special case of umbrella order with $q=1$. Here we
test
\begin{equation}\label{E:hormesis}
H_0: \bpi_1= \leqst \cdots \leqst\bpi_G \quad\text{versus}\quad
H_a:\bpi_1 \geqst \cdots \geqst \bpi_q \leqst \bpi_{q+1} \leqst \cdots \leqst \bpi_G , q>1.
\end{equation}
We use parametric bootstrap to resample under $H_0$: we estimate $\bpi_i$ assuming
stochastic ordering, then based on this distribution generate random samples with 
the same cluster- and group-sizes as the observed data.

\texttt{hormesis.LRT} calculates the likelihood-ratio test statistic for testing
\eqref{E:hormesis}.
@o Reprod.R @{
hormesis.LRT <- function(cbdata, control=mixControl()){

  # LL separately for each q (turn):
  # turn=1 is H0, turn>1 is Ha
  ntrt <- nlevels(cbdata$Trt)
  ll <- numeric(ntrt)
  for (turn in 1:ntrt){
    res <- mix.reprod.mle(cbdata, turn=turn, control=control)
    ll[turn] <- res$loglik
  }
  ll0 <- ll[1]
  ll1 <- max(ll)
  est.turn <- which.max(ll)
  lrt <- 2*(ll1 - ll0)
  attr(lrt, "ll0") <- ll0
  attr(lrt, "ll1") <- ll1
  c(lrt, est.turn)  
...
\texttt{simul} generates a new data set based on the marginal frequencies
in \texttt{cbdata} and probabilities in \texttt{probs}. Both should be data sets
with columns \texttt{Trt}, \texttt{ClusterSize} and \texttt{NResp}; in addition
\texttt{cbdata} should have \texttt{Freq} and \texttt{probs} should have \texttt{Prob}. 
@O Reprod.R @{
simul <- function(cbdata, probs){
   cbdata <- merge(cbdata, probs, all=T)
   cbdata$Freq[is.na(cbdata$Freq)] <- 0
   ran.gen <- function(idx, d){
     tmp <- rmultinom(n=1, size=sum(d$Freq[idx]), prob=d$Prob[idx])[,1]
     cs <- d$ClusterSize[idx[1]]
     cbind(Freq=tmp, NResp=0:cs, ClusterSize=cs, Trt=d$Trt[idx[1]])}
   
   a <- tapply(1:nrow(cbdata), list(ClusterSize=cbdata$ClusterSize, Trt=cbdata$Trt), ran.gen, d=cbdata)
   a <- data.frame(do.call(rbind, a))
   a$Trt <- factor(a$Trt, labels=levels(cbdata$Trt))
   subset(a, Freq>0) 
 }
@| simul @}

Finally, \texttt{boot.hormesis.LRT} estimates the p-value of the test.
@O Reprod.R @{
boot.hormesis.LRT <- function(cbdata, R=100, control=mixControl()){
  stoch.est <- mix.reprod.mle(cbdata, control=control)
  est.d <- stoch.est$MLest
  
  res <- boot(cbdata, hormesis.LRT, sim="parametric", R=R, 
              ran.gen=simul, control=control, mle=est.d)
  p <- mean(res$t[,1] >= res$t0)
  LRT <- res$t0
  list(LRT=LRT, p.val=p, boot.res=res)}           
@| boot.hormesis.LRT @}
...
@O Reprod.R @{
NOSTASOT <- function(cbdata, R=100, sig.level=0.05, control=mixControl()){
   ntrt <- nlevels(cbdata$Trt)
   ...
   while (!NOSTASOT.found & (curr.gr.idx>1)){
     ...
     so.test <- perm.LRT(d1,  R=R, control=control)
     p.vec[curr.gr] <- so.test$p.val
     if (so.test$p.val < sig.level){ #NOSTASOT not found yet
...
@O Reprod.R @{
trend.test <- function(cbdata, test=c("RS","GEE","GEEtrend","GEEall","SO"), exact=test=="SO", R=100, control=mixControl()){ 
   test <- match.arg(test)
   if (!exact && !(test=="SO")){
     ...
     boot.LRT.fun <- function(dat, idx){
       ...
       stat <- switch(test, SO=null.LRT(dat.f, control=control),
