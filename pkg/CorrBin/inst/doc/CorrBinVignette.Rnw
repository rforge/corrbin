...
<<SOtest>>=
  set.seed(4461)
 (so.res <- trend.test(sh, test="SO", R=100, control=mixControl(eps=0.1, max.directions=40)))
@
...
\texttt{mix.mc.mle} function:
<<SOmle>>=
   sh.SO.est <- mix.mc.mle(sh, control=mixControl(eps=0.1, max.directions=40))
   head(sh.SO.est$MLest)
@
\begin{figure}
<<SOmleplot, fig=true, width=8, height=6>>=
   print(xyplot(Prob~NResp|factor(ClusterSize), groups=Trt, data=sh.SO.est$MLest, 
